...
| 发帖任务 | 可拉取任务并一键触发后台发布 | 使用后台发布接口执行图文/视频 |
| 搜索任务 | 可创建、查看列表和详情；即时搜索默认走 `worker_search` 小号池 | 继续补齐更多搜索筛选与结果能力 |
| 搜索详情转发帖 | 帖子详情页可直接调用后台创建发帖任务 | 后续补充任务模板和定时发布 |
| 账号信息 | 展示头像、昵称、关注/粉丝/获赞和已发布笔记列表 | 增加更多账号资产指标 |
...
| 首页发帖 Tab | 拉取后台创建的发帖任务，一键调用后台发布接口 |
| 首页搜索 Tab | 即时搜索、创建关键词任务、查看任务列表、点击帖子进入详情页，并可在详情页右上角直接创建发帖任务 |
| 首页我的 Tab | 查看主账号资料、已发布笔记列表；通过二级设置页执行语言切换、刷新并校验、退出登录、维护后台地址 |
...
4. App 不记录明文 Cookie。
5. 搜索和“我的”页的公开资料、帖子列表优先复用 `worker_search` 小号 Cookie，减少主账号请求量。
6. 搜索详情字段优先复用搜索接口返回数据，不对每条结果追加详情请求。
7. Cookie 校验失败后，不继续执行搜索、发帖和账号查询动作。
